...
for(i in 1:5){
  ...
  BEL_ind_mix_100[[i]]<-clusterApply(cl=cluster, x=1:3, function(z){BEL_leroux_new(y,x,n,p,var,rho=0,niter=1000000,
                                                                                  beta_init, psi_init, tau_init,R, wi, sd_psi=0.9, 
                                                                                  sd_beta=1.2, sd_tau=0.4)})

}
save(BEL_ind_mix_100,file="Results/BEL_ind_mix_100.RData")
