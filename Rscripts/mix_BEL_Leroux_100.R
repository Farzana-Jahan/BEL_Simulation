...
require(dplyr)          # For inner_join(), bLeroux_rows(),### between(), mutate()
...
for(i in 1:5){
  ...
  BEL_Leroux_mix_100[[i]]<-clusterApply(cl=cluster, x=1:3, function(z){BEL_leroux_new(y,x,n,p,var,rho=0.9,niter=1000000,
                                                                                  beta_init, psi_init, tau_init,R, wi, sd_psi=0.09, 
                                                                                  sd_beta=0.6, sd_tau=0.4)})
  
  }
save(BEL_Leroux_mix_100,file="Results/BEL_Leroux_mix_100.RData")
