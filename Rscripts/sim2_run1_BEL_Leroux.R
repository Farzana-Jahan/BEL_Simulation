...
W<-readRDS("Data/sim_data/W_25areas.RDS")
...
for(i in 1:5){
  ...
  BEL_Leroux2_sim_25[[i]]<-clusterApply(cl=cluster, x=1:3, function(z){BEL_leroux_new(y,x,n,p,var,rho=1, niter=1000000,
                                                                                     beta_init, psi_init, tau_init,R, wi, sd_psi=0.4, 
                                                                                     sd_beta=1.7, sd_tau=0.6)})
  
 
}

save(BEL_Leroux2_sim_25,file="Results/BEL_Leroux2_sim_25_sim2.RData")
